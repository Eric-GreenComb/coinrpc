...
// 创建钱包地址
func (o *OmniClient) GetNewAddress(account string) (address string) {

	c := rpc.NewClient(o.ConnCfg)

	if err := c.Call(&address, "getnewaddress", account); err != nil {
...
func (o *OmniClient) FundedSendall(fromaddress, toaddress string, ecosystem uint32, feeaddress string) (string, error) {
	c := rpc.NewClient(o.ConnCfg)
	var hash string
	if err := c.Call(&hash, "omni_funded_sendall", fromaddress, toaddress, ecosystem, feeaddress); err != nil {
		return hash, err
	}
...
func (o *OmniClient) GetBlockTransactions(index int64) (result []string) {
	...
	if err := c.Call(&result, "omni_listblocktransactions", index); err != nil {
		log.Printf("Call %v", err)
	}
...
func (o *OmniClient) GetTransaction(txid string) (result Tx) {
